...
void system_init(void)
{
	...
	*(lFFL1StartAddress + 5) = 2;  //400 MHz
	// Soc Divisor
	*(lFFL1StartAddress + 6) = 3;  //200 MHz
	// EFPGA Divisor
	*(lFFL1StartAddress + 7) = 6;  //100 MHz
	// Ref Divisor
	*(lFFL1StartAddress + 8) = 1;  //10 MHz
#if VERISILICON_PLL == 1

	//FLL Control Register
	*lFFL1StartAddress = 0x03000103; // PD, PDDP, Refdiv=1, Reset, ByPASS
	// Divisor Registe
	*(lFFL1StartAddress + 1) = 0x00780002; // 1200 PLL ouput divisor=2
	//Fractional Reg
	*(lFFL1StartAddress + 2) = 0;
	// Spread Spectrum 1 Register SSRate
	*(lFFL1StartAddress + 3) = 0;
	// Spread Spectrum 2 Register SSlope
	*(lFFL1StartAddress + 4) = 0;
	// Soc Divisor
	*(lFFL1StartAddress + 5) = 2;  //300 MHz
	// Soc Divisor
	*(lFFL1StartAddress + 6) = 3;  //200 MHz
	// EFPGA Divisor
	*(lFFL1StartAddress + 7) = 6;  //100 MHz
	...
	*lFFL1StartAddress = 0x00000101;  // Refdiv=1, ByPASS

	// should wait for Lock

	*lFFL1StartAddress = 0x00000100;  // Refdiv=1 clocks on
